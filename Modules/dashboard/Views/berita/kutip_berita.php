<div id="content-berita">
    <h1 class="mt-4">Kutip Berita</h1>
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="<?= base_url('dashboard') ?>">Dashboard</a></li>
        <li class="breadcrumb-item">Berita & Artikel</li>
        <li class="breadcrumb-item active">Kutip Berita</li>
    </ol>
...
<script>
    $(document).ready(function() {
        $('#table-berita').DataTable({
            searching: true,
            responsive: true,
            ajax: "<?= base_url('berita/list_kutip_berita'); ?>"
        })
    })

    function tambahBerita() {
        $.ajax({
            url: "<?= base_url('berita/tambahKutipBerita') ?>",
            method: 'GET',
    ...
    function edit(id) {
        $.ajax({
            url: "<?= base_url('berita/editKutipBerita') ?>/" + id,
            method: 'GET',
            success: function(data) {
                $('#content-berita').html(data);
            }
